    ...
    <script>
        ...
        function renderGrid() {
            ...
            tokens.forEach((tok, ti) => {
                const display = (tok.display || tok.text).replace(/</g, '&lt;').replace(/>/g, '&gt;');
                ...
                for (let li = n_layers - 1; li >= 0; li--) {
                    ...
                    const top = preds[0];
                    const topDisplay = JSON.stringify(top.token).slice(1,-1).replace(/</g,'&lt;');
                    const others = preds.slice(1).map(p =>
                        JSON.stringify(p.token).slice(1,-1).replace(/</g,'&lt;')
                    ).join(', ');
        ...
        function selectCell(ti, li) {
            ...
            const info = preds.map((p,i) => `${i+1}. "${p.token}" (${p.logit.toFixed(1)})`).join('  ');

            const bar = document.getElementById('info-bar');
            bar.innerHTML = `<strong>token ${ti}:</strong> "${tok.display}"  <strong>layer ${li}:</strong> ${info}`;
...
def main():
    ...
    print(f"\nLogit Lens Viewer running at http://{args.host}:{args.port}")
